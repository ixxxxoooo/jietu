...
> ```
>
> 或者：右键 **Jietu.app** → **打开** → **打开**。

### 授权

1. **屏幕录制**（必需）— **系统设置 › 隐私与安全性 › 录屏与系统录音**，把 Jietu 拖进去（或点 **+** 选中它）并打开开关，然后 **重启应用**（macOS 要求重启后才生效）。如果列表里**已经有 Jietu 却仍显示未授权**，那一行是升级前留下的旧记录：选中它按 **−** 删掉，重新加一次，再重启应用。
