...
- 声音只支持**系统声音**（页面里的视频 / 音乐），默认关，AAC 128 kbps；**不支持麦克风**
- 鼠标光标会录进画面
...
### 标注编辑快捷键（编辑器内，默认已配好）
- 撤销 / 重做：默认 **⌘Z / ⇧⌘Z**，「原地编辑」与「独立窗口」两个编辑器里都生效
...
- 撤销 / 重做（快照式，连带底图一起回滚）：默认 **⌘Z / ⇧⌘Z**，原地编辑与独立窗口都支持，键位可在设置页改
...
- Quick Access 浮窗：**图片卡**只显示图片预览，悬停出现四角圆形按钮（复制 / 标注 / 钉图 / 关闭）+ 中间保存；
  **视频卡**（录屏收工）封面正中挂「▶ 时长」胶囊，中央「播放」，四角是复制文件 / 另存为… / 在访达中显示 / 关闭
...
| 录屏 | 录制系统声音（关）、帧率（30 / 60，默认 30）、输出位置 + 在访达中显示 |
| 标注 | 编辑方式（原地编辑 / 独立窗口）、默认样式摘要 + 恢复默认 |
