...
- 五个动作（区域 / 窗口 / 全屏 / 定时 / 滚动长图）出厂都不绑定快捷键，避免抢占系统与其它 App 的组合键
- 点「未设置」按组合键即录入，「清除」可随时删掉；与其它 App 或本 App 其它动作撞车时自动回滚并提示

### 滚动长图
- 菜单「滚动长图…」或自配热键 → 框选区域 → 控制条浮在选区下方
- 手动匀速滚动内容，`ScrollStitcher` 用行签名互相关算出纵向位移并逐帧拼接
- 停止滚动约 1.5 秒自动完成，也可点「完成」/ Esc 结束；支持撤销前的整图预览
...
- 文字原地输入、双击改文案；颜色、线宽、字号、马赛克块大小、模糊半径、放大倍率均可事后修改
